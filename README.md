...
1. 由于huggingface连接经常异常，需要先下载到本地，然后上传到服务器，但是由于文件中的软连接问题，会导致一些棘手的现象，因此最好的方式是通过windows的WSL下载文件，然后通过scp[传输数据](https://blog.csdn.net/qq_43674360/article/details/126096275)，命令为：
   scp -r {target_files} {user}@host:{target_dir}，其中-r是传输目录的参数
